  ...
  <tr class="table_date"><!-- 한달치 데이터만 볼 수 있도록 연도와 월만 체크 가능 db에서 꺼내올때 가져올 수 있는가? -->
    <td colspan="4" align="center">
      ...
      <select name="table_year" onchange="javascript:get_date(this.value)" >
        <option> 연도선택 </option>
        <option value="2021" <% if ("2021".equals(yearStr)) {%> selected <%}%>>2021년</option> <!-- 데이터값이 변경되었으면 좋겠음 /선택한 연도로 -->
...
%>
  <tr class="table_date"><!-- 한달치 데이터만 볼 수 있도록 연도와 월만 체크 가능 db에서 꺼내올때 가져올 수 있는가? -->
    <td colspan="4" align="center">
      ...
      <select class="table_year" name="table_year" onchange="javascript:get_date(this.value)">
        <option> 연도선택 </option>
        <option value="2021" <% if ("2021".equals(yearStr)) {%> selected <%}%>>2021년</option> <!-- 데이터값이 변경되었으면 좋겠음 /선택한 연도로 -->
        <option value="2020" <% if ("2020".equals(yearStr)) {%> selected <%}%>>2020년</option>
      </select>
...
%>
  <tr class="data_infected_count"><!-- 감염자 수 눌렀을 경우 -->
    ...
    <td width="50%" colspan="2">
      <span><%=list.get(i).getInfected_count() %>명</span>
    </td>
    <td width="25%" id="date">
      <span onmouseover="showIssue()"><%=list.get(i).getDate() %></span>
      <div class="issueOn">
        <h3> title</h3>
        <p> description</p>
        <span> date</span>
...
%>
  <tr class="data_healing_count"><!-- 격리해제자 수 눌렀을 경우 -->
    ...
    <td width="50%" colspan="2">
      <span><%=list.get(j).getHealing_count() %>명</span>
    </td>
    <td width="25%" id="date">
...
%>
  <tr class="data_deaths_count"><!-- 사망자 수 눌렀을 경우-->
    ...
    <td width="50%" colspan="2">
      <span><%=list.get(k).getDeaths_count() %>명</span>
    </td>
    <td width="25%" id="date">
